...
exports.getRatesHandler = exports.setRateHandler = void 0;
// Импортируем модель для работы с базой данных курсов
const CurrencyRate_model_1 = __importDefault(require("@/models/CurrencyRate.model"));
...
const setRateHandler = async (ctx) => {
    ...
    const code = args[0].toUpperCase(); // Приводим код к верхнему регистру
    ...
    const allowedCodes = ["KRW", "CNY", "JPY", "EUR"];
    if (!allowedCodes.includes(code)) {
        await ctx.reply(`🚫 Неверный код валюты "<b>${code}</b>".\n` +
            `Допустимые коды: <b>${allowedCodes.join(", ")}</b>`, { parse_mode: "HTML" });
        return;
    }
    // Валидация значения курса: проверяем, что это положительное число
    if (isNaN(rate) || rate <= 0) {
        await ctx.reply(`🚫 Неверное значение курса "<b>${args[1]}</b>".\n` +
            `Укажите положительное число (можно с точкой или запятой).`, { parse_mode: "HTML" });
        return;
    }
    // Пытаемся обновить или создать запись в базе данных
    try {
        console.log(`[Admin Bot] Attempting to set rate for ${code} to ${rate} by user ${ctx.from?.id}`);
        ...
        const updatedRateDoc = await CurrencyRate_model_1.default.findOneAndUpdate({ code: code }, // Условие поиска: найти по коду валюты
        ...
        console.log(`[Admin Bot] Successfully set rate for ${updatedRateDoc.code} to ${updatedRateDoc.rate}. Updated at: ${updatedRateDoc.updatedAt}`);
        // Отправляем подтверждение пользователю
        await ctx.replyWithHTML(`✅ Курс для <b>${updatedRateDoc.code}</b> успешно установлен: <b>${updatedRateDoc.rate}</b>\n` +
            `<i>Последнее обновление: ${updatedRateDoc.updatedAt.toLocaleString("ru-RU")}</i>`);
    }
    catch (error) {
        // Обработка ошибок при работе с базой данных
        console.error(`[Admin Bot] Error setting rate for ${code} by admin ${ctx.from?.id}:`, error);
        // Отправляем сообщение об ошибке пользователю
        await ctx.reply(`❌ <b>Ошибка при установке курса!</b>\n\n` +
            `<i>Детали: ${error.message || "Неизвестная ошибка базы данных"}</i>\n\n` +
            `Попробуйте еще раз или проверьте формат команды.`, { parse_mode: "HTML" });
    }
};
exports.setRateHandler = setRateHandler;
// --- Команда /getrates ---
// Предназначена для просмотра всех текущих курсов, сохраненных в базе данных.
const getRatesHandler = async (ctx) => {
    // Middleware isAdmin уже проверил права доступа.
    ...
    try {
        ...
        const rates = await CurrencyRate_model_1.default.find().sort({ code: 1 }).exec();
        // Проверяем, есть ли вообще записи в базе данных
        if (!rates || rates.length === 0) {
            await ctx.reply("⚠️ В базе данных пока нет установленных курсов.\n" +
                "Используйте команду <code>/setrate КОД ЗНАЧЕНИЕ</code> для их добавления.", { parse_mode: "HTML" });
        ...
        console.error(`[Admin Bot] Error fetching rates by admin ${ctx.from?.id}:`, error);
        await ctx.reply(`❌ <b>Ошибка при получении курсов!</b>\n\n` +
            `<i>Детали: ${error.message || "Неизвестная ошибка базы данных"}</i>`, { parse_mode: "HTML" });
    }
};
exports.getRatesHandler = getRatesHandler;
